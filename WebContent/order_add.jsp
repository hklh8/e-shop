							...
							<table width="100%" class="border_table" id='payment_form'>
								...
								<tr>
									<th><label><input class="radio" name="order.payType"
											alt="0" title="预存款支付" type="radio" value="预存款支付" checked />预存款支付</label></th>
									<td>支付手续费：￥0</td>
								</tr>
								<tr>
									<th><label><input class="radio" name="order.payType"
							...
							<table width="100%" class="cart_table t_c">
								...
								<tbody>
									...
									<c:forEach items="${orderDetails}" var="orderDetail" varStatus="s">
										<input type="hidden" name="orderDetails[${s.index}].goodsId" value="${orderDetail.goods.id}">
										...
										<tr>
											...
											<c:set var="totalMoney" value="${totalMoney+orderDetail.goods.price2*orderDetail.nums }"></c:set>
										</tr>
									</c:forEach>
									<!-- 商品展示 结束-->
